...
KNOB<BOOL> KnobSrcDebug(KNOB_MODE_WRITEONCE, "pintool",
    "src_debug", "0", 
    "Enable verbose source-location diagnostics.");
...
struct Region {
    ...
    string  alloc_file;
    INT32   alloc_line;
    Region() : start(0), size(0), tag("-"), alloc_file("?"), alloc_line(0)  {}
};
...
static FILE* tracef  = nullptr;          // pinatrace.out (alloc/free + loads/stores)
...
// Adding thread role to the thread
enum ThreadRole {
    ROLE_UNKNOWN = 0,
    ROLE_WORKER,
    ROLE_OTHER
...
static inline bool CallocPush(ThreadCtx* tc, size_t n, size_t sz, ADDRINT ip, UINT64& outSeq)
{
    ...
    tc->pendingCallocTop++; // increment AFTER write
    return true;
}
...
static inline bool CallocPop(ThreadCtx* tc, ThreadCtx::PendingCalloc& out)
{
    ...
    tc->pendingCallocTop--;            // decrement BEFORE read
...
// cached version for GetImgFromIp
...
static inline void GetSrcFromIp(ADDRINT ip, std::string &file, INT32 &line, INT32 &col)
{
    ...
    // 4. Fallback to addr2line at exact image-relative offset
    if (!imgName.empty()) {
    ...
    // 5. final try: addr2line at ip-1 offset
    if (ip > 0) {
        std::string imgName2;
        ADDRINT imgOff2 = 0;
...
static inline std::string BaseSym(std::string n) {
    // truncate version suffix: malloc@@GLIBC_2.2.5 -> malloc
    size_t at = n.find('@');
    if (at != std::string::npos) n = n.substr(0, at);

    // truncate common glibc prefixes: __GI___libc_malloc -> __libc_malloc
...
static BOOL OnSigUsr2(THREADID tid, INT32 sig, CONTEXT* ctxt, BOOL hasHandler, const EXCEPTION_INFO* pExceptInfo, VOID* v)
{
...
static VOID RecordRead(THREADID tid, VOID* ip, VOID* ea, UINT32 bytes)
{
    ...
    // only if knob trace_stack=1 lookup into stack regiions
...
static VOID RecordWrite(THREADID tid, VOID* ip, VOID* ea, UINT32 bytes)
{
    ...
    size_t off = 0;
    bool found = FindRegionWithOff(a, snap, off);
    // only when knob trace_stack=1 lookup into stack regiions
    if (!found && KnobTraceStack.Value()) {
        found = FindStackRegionWithOff(tid, a, snap, off);
    }
...
static BOOL ShouldInstrumentIns(INS ins) {
    ...
    // find the img that contains this instruction
    IMG img = IMG_FindByAddress(INS_Address(ins));
    if (!IMG_Valid(img)) return FALSE;
    
    // always instrument the main executable 
    if (IMG_IsMainExecutable(img)) return TRUE;

    // (even if main_exe_only is ON)
    if (!KnobMainExeOnly.Value()) return TRUE;

    // if trace_libc_memops=1 intrument libc
...
// callbacks for memory reads/writes
...
static VOID AfterMalloc(ADDRINT ret, size_t sz, ADDRINT caller_ip) {
    ...
    // guard against recursive malloc calls
...
static VOID AfterRealloc(ADDRINT ret, ADDRINT oldp, size_t sz, ADDRINT caller_ip) {
    ...
    if (oldp) {
        ...
        if (tracef) {
            if (known) {
                        PrintFreeKnown(tid,(ADDRINT)snap.start,snap.size,snap.tag.c_str(), (ADDRINT)oldp,
...
//update malloc/calloc/realloc/free libc
static VOID HookLibcAllocators(IMG img) {
    ...
    for (SEC s = IMG_SecHead(img); SEC_Valid(s); s = SEC_Next(s)) {
     ...
     // Hook only allocator-related libs
...
static VOID ImageLoad(IMG img, VOID*) {
    ...
    HookMemcachedThreadRoles(img);
    TrackImageGlobals(img);
...
static VOID ThreadStart(THREADID tid, CONTEXT* ctxt, INT32, VOID*) {
    ...
    PIN_ReleaseLock(&g_events_lock);
        ...
        ADDRINT sp = 0;
            ...
            const size_t kApproxStackSize = 8 * 1024 * 1024;
